...
# Funkcja do generowania sygnału modulowanego FM
def plot_fm_modulation(carrier_freq=10, carrier_ampl=1, mod_freq=2, mod_index=2, mod_ampl=1):
    ...
    # Sygnały początkowe
    mod_signal = mod_ampl * np.sin(2 * np.pi * mod_freq * t)  # Sygnał modulujący
    modulated_signal = carrier_ampl * np.sin(2 * np.pi * carrier_freq * t + mod_index * mod_signal)  # Modulacja FM

    # Ustawienia subplotów
    ax[0, 0].set_title("Sygnał nośny")
    ax[0, 0].set_xlabel("Czas [s]")
    ax[0, 0].set_ylabel("Amplituda")
    carrier_line, = ax[0, 0].plot(t, carrier_ampl * np.sin(2 * np.pi * carrier_freq * t), label="Sygnał nośny")
    ax[0, 0].grid(True)
    ax[0, 0].legend()
    ...
    ax[1, 0].set_title("Sygnał modulowany (FM)")
    ax[1, 0].set_xlabel("Czas [s]")
    ax[1, 0].set_ylabel("Amplituda")
    modulated_line, = ax[1, 0].plot(t, modulated_signal, color='green', label="Sygnał modulowany (FM)")
    ax[1, 0].grid(True)
    ax[1, 0].legend()
    ...
    mod_ampl_slider = Slider(slider_ax4, 'A. modulacji', 0.1, 2, valinit=mod_ampl)
    ...
    def update(val):
        ...
        modulated_signal = carrier_ampl * np.sin(2 * np.pi * carrier_freq * t + mod_index * mod_signal)

        carrier_line.set_ydata(carrier_ampl * np.sin(2 * np.pi * carrier_freq * t))
