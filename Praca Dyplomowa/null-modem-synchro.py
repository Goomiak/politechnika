...
class RS232Monitor(QMainWindow):
    def __init__(self):
        ...
        self.prbs_data = SYNC_HEADER + generate_prbs(1024)
        self.rx_buffer = bytearray()
        self.sniff_buffer = bytearray()
    ...
    def sync_and_check(self, buffer, data, expected, total_received, total_errors):
        buffer.extend(data)
        while len(buffer) >= len(expected):
            ...
            if idx == -1:
                buffer.clear()
                break
            if idx + len(expected) > len(buffer):
                break
            payload = buffer[idx:idx+len(expected)]
            errors = sum(1 for a, b in zip(payload, expected) if a != b)
            total_received += len(payload)
            total_errors += errors
            del buffer[:idx+len(expected)]
        return total_received, total_errors

    def receive_data(self):
        while self.running:
            try:
                data = self.rx_port.read(256)
                self.rx_received, self.rx_errors = self.sync_and_check(
                    self.rx_buffer, data, self.prbs_data, self.rx_received, self.rx_errors)
            except Exception as e:
    ...
    def sniff_data(self):
        while self.running:
            try:
                ...
                self.sniff_received, self.sniff_errors = self.sync_and_check(
                    self.sniff_buffer, data, self.prbs_data, self.sniff_received, self.sniff_errors)
            except Exception as e:
