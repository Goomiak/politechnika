...
class RS232Monitor(QMainWindow):
    ...
    def populate_ports(self):
        ...
        # Keep previously selected ports if still available
    ...
    def receive_data(self):
        while self.running:
            try:
                ...
                expected = generate_prbs(256)
                self.rx_errors += sum(b1 != b2 for b1, b2 in zip(data, expected))
            except Exception as e:
    ...
    def sniff_data(self):
        while self.running:
            try:
                ...
                expected = generate_prbs(256)
                self.sniff_errors += sum(b1 != b2 for b1, b2 in zip(data, expected))
            except Exception as e:
    ...
    def update_stats(self):
        if self.start_time:
            ...
            rx_ber = self.rx_errors / (self.rx_received * 8) if self.rx_received > 0 else 0
            sniff_ber = self.sniff_errors / (self.sniff_received * 8) if self.sniff_received > 0 else 0
            self.stats_label.setText(
                f"Time: {duration:.1f}s | RX BER: {rx_ber:.2e} | Sniff BER: {sniff_ber:.2e}"
            )
