...
plt.subplots_adjust(bottom=0.3, hspace=0.5)  # Zwiększono odstęp między wykresami i dodano miejsce na suwaki oraz przycisk

# Ustawienia okna - wyśrodkowanie
manager = plt.get_current_fig_manager()
try:
    manager.resize(1000, 800)  # Ustawienia wymiarów okna
    screen_width = manager.window.winfo_screenwidth()
    screen_height = manager.window.winfo_screenheight()
    new_x = (screen_width - 1000) // 2
    new_y = (screen_height - 800) // 2
    manager.window.wm_geometry(f"+{new_x}+{new_y}")
...
amp_slider_ax = plt.axes([0.2, 0.15, 0.65, 0.03], facecolor=axcolor)
freq_slider_ax = plt.axes([0.2, 0.1, 0.65, 0.03], facecolor=axcolor)
