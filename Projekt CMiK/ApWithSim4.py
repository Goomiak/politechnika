...
def show_direct_detection(menu_window, final_button):
    slides = [
        ("image_text", "direct1.png", "Detekcja bezpośrednia", "Detekcja bezpośrednia sygnałów optycznych polega na bezpośrednim pomiarze mocy promieniowania optycznego przy użyciu detektorów, takich jak fotodiody czy fotopowielacze. Proces ten polega na konwersji fotonów padających na detektor na sygnał elektryczny, którego natężenie jest proporcjonalne do mocy optycznej padającego światła."),
        ("image_text", "direct4.png", "Zasada działania", "W detekcji bezpośredniej detektor (np. fotodioda) generuje sygnał elektryczny odpowiadający intensywności padającego światła. Główne zasady detekcji bezpośredniej są oparte na proporcjonalności między liczbą fotonów a generowanym prądem. Typowo detekcja bezpośrednia wykorzystuje efekty wewnętrzne, takie jak efekt fotoelektryczny w fotodiodach."),
        ("simulation", "direct_sim_1", "Detekcja bezpośrednia", "Interaktywna symulacja - wprowadź parametry:"),
    ]
...
def show_coherent_detection(menu_window, final_button):
    slides = [
        ("image_text", "coherent2.png", "Detekcja koherentna", "Detekcja koherentna sygnałów optycznych polega na interferencyjnym odbiorze sygnału optycznego za pomocą jego zmieszania z lokalnym oscylatorem (LO) o tej samej częstotliwości lub zbliżonej do częstotliwości sygnału. Dzięki temu możliwe jest wydobycie fazy i amplitudy sygnału, co umożliwia większą czułość oraz poprawę jakości informacji o przenoszonym sygnale."),
        ("simulation", "coherent_sim_1", "Detekcja koherentna", "Interaktywna symulacja wprowadź parametry:"),
    ]
    show_window_sequence(slides, "Coherent Detection", lambda: mark_coherent_slides_as_viewed(menu_window, final_button))

def show_summary(menu_window, final_button):
    slides = [
        ("simulation", "Detekcja subkoherentna", "Detekcja subkoherentna", "Interaktywna symulacja wprowadź parametry:"),
        ("image_text", "subcoherent1.png", "Podsumowanie", "Detekcja bezpośrednia jest idealna dla prostych systemów o krótkim zasięgu, natomiast detekcja koherentna znajduje zastosowanie tam, gdzie wymagana jest wysoka czułość i zaawansowana modulacja. Detekcja subkoherentna stanowi kompromis dla systemów o średnich wymaganiach, oferując pewne możliwości modulacyjne przy niższych wymaganiach sprzętowych niż pełna detekcja koherentna.")
       
    ]
    show_window_sequence(slides, "Summary", lambda: mark_coherent_slides_as_viewed(menu_window, final_button))
...
def show_window_sequence(slides, title, on_finish):
    def create_slide_window(index):
        ...
        if slide_type == "image_text":
            ...
            # image = image.resize((600, 200), Image.Resampling.LANCZOS)  # Dostosowanie rozmiaru obrazu
            ...
            param_frequency = tk.DoubleVar(value=10.0)  # W MHz
            param_lo_frequency = tk.DoubleVar(value=11.0)  # W MHz

            def update_parameters():
                # Usuwanie istniejących widgetów
                for widget in param_frame.winfo_children()[1:]:
                    widget.destroy()

                # Detekcja bezpośrednia
                if slide_title == "Detekcja bezpośrednia":
                    tk.Label(param_frame, text="Amplituda").pack()
                    tk.Entry(param_frame, textvariable=param_amplitude).pack()

                    tk.Label(param_frame, text="Częstotliwość (MHz)").pack()
                    tk.Entry(param_frame, textvariable=param_frequency).pack()

                # Detekcja koherentna i subkoherentna
                elif slide_title in ["Detekcja koherentna", "Detekcja subkoherentna"]:
                    tk.Label(param_frame, text="Częstotliwość (MHz)").pack()
                    tk.Entry(param_frame, textvariable=param_frequency).pack()

                    tk.Label(param_frame, text="Częstotliwość lokalnego oscylatora (MHz)").pack()
                    tk.Entry(param_frame, textvariable=param_lo_frequency).pack()

                plot_simulation()  # Aktualizacja wykresu po zmianie parametrów

            def plot_simulation():
                amplitude = param_amplitude.get()
                frequency = param_frequency.get() * 1e6  # Przeliczenie na Hz
                lo_frequency = param_lo_frequency.get() * 1e6  # Przeliczenie na Hz

                t = np.linspace(0, 1e-6, 1000)  # Czas w mikrosekundach
                signal = amplitude * np.cos(2 * np.pi * frequency * t + np.pi / 4)

                ax.clear()

                if slide_title == "Detekcja bezpośrednia":
                    detected_power = signal**2
                    ax.plot(t * 1e6, signal, label="Sygnał optyczny")  # Czas w μs
                    ax.plot(t * 1e6, detected_power, label="Moc sygnału (detekcja bezpośrednia)")
                    ax.set_title("Detekcja bezpośrednia")
                elif slide_title == "Detekcja koherentna":
                    local_oscillator = amplitude * np.cos(2 * np.pi * lo_frequency * t)
                    mixed_signal = signal + local_oscillator
                    detected_current = mixed_signal**2
                    ax.plot(t * 1e6, signal, label="Sygnał optyczny")
                    ax.plot(t * 1e6, local_oscillator, label="Lokalny oscylator")
                    ax.plot(t * 1e6, detected_current, label="Prąd wyjściowy (detekcja koherentna)")
                    ax.set_title("Detekcja koherentna")
                elif slide_title == "Detekcja subkoherentna":
                    f_diff = frequency - lo_frequency
                    subcoherent_signal = signal + amplitude * np.cos(2 * np.pi * f_diff * t)
                    detected_subcoherent = subcoherent_signal**2
                    ax.plot(t * 1e6, signal, label="Sygnał optyczny")
                    ax.plot(t * 1e6, subcoherent_signal, label="Sygnał po zmieszaniu (subkoherentny)")
                    ax.plot(t * 1e6, detected_subcoherent, label="Prąd wyjściowy (detekcja subkoherentna)")
                    ax.set_title("Detekcja subkoherentna")

                ax.set_xlabel("Czas [μs]")
                ax.set_ylabel("Amplituda / Moc")
                ax.legend()
                canvas.draw()

            # Automatyczna aktualizacja wykresu przy zmianie parametrów
            param_amplitude.trace("w", lambda *_: plot_simulation())
            param_frequency.trace("w", lambda *_: plot_simulation())
            param_lo_frequency.trace("w", lambda *_: plot_simulation())
        ...
        #     # Parametry symulacji
        #     param_amplitude = tk.DoubleVar(value=1.0)
        #     param_frequency = tk.DoubleVar(value=10000000)
        ...
        #         signal = amplitude * np.cos(2 * np.pi * frequency * t + np.pi / 4)
        #         local_oscillator = amplitude * np.cos(2 * np.pi * (frequency + 1e6) * t)

        #         if slide_title == "Detekcja bezpośrednia":
        #             detected_power = signal**2
        #             ax.clear()
        #             ax.plot(t, signal, label="Sygnał optyczny")
        #             ax.plot(t, detected_power, label="Moc sygnału (detekcja bezpośrednia)")
        #             ax.legend()
        #         elif slide_title == "Detekcja koherentna":
        #             mixed_signal = signal + local_oscillator
        #             detected_current = mixed_signal**2
        #             ax.clear()
        #             ax.plot(t, signal, label="Sygnał optyczny")
        #             ax.plot(t, local_oscillator, label="Lokalny oscylator")
        #             ax.plot(t, detected_current, label="Prąd wyjściowy (detekcja koherentna)")
        #             ax.legend()
        #         elif slide_title == "Detekcja subkoherentna":
        #             f_diff = frequency - (frequency + 1e6)
        #             subcoherent_signal = signal + amplitude * np.cos(2 * np.pi * f_diff * t)
        #             detected_subcoherent = subcoherent_signal**2
        #             ax.clear()
        #             ax.plot(t, signal, label="Sygnał optyczny")
        #             ax.plot(t, subcoherent_signal, label="Sygnał po zmieszaniu (subkoherentny)")
        #             ax.plot(t, detected_subcoherent, label="Prąd wyjściowy (detekcja subkoherentna)")
        #             ax.legend()
...
def create_menu_window():
    ...
    def on_summary():
        show_summary(menu_window, btn_final)
