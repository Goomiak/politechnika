...
def update(val):
    ...
    frequency_thz = freq_slider.val  # Wartość w THz na suwaku
    frequency_hz = frequency_thz * 1e12  # Konwersja na Hz

    # Aktualizacja sygnału optycznego
    E_t = amplitude * (1 + 0.5 * np.sin(2 * np.pi * f_mod * t)) * np.sin(2 * np.pi * frequency_hz * t)
...
fs = 1e3  # Częstotliwość próbkowania (Hz)
t = np.linspace(0, 1e-6, int(fs))  # Oś czasu w mikrosekundach
f_carrier = 50e12  # Początkowa częstotliwość nośna (Hz)
f_mod = 5  # Częstotliwość modulacji (Hz)
amplitude = 1  # Początkowa amplituda
responsivity = 0.5  # Responsywność fotodetektora (R)
cutoff_freq = 20e12  # Częstotliwość odcięcia filtra (Hz)
...
line_E, = axs[0].plot(t * 1e6, E_t, label='E(t)')  # Czas w mikrosekundach
axs[0].set_xlabel('Czas (μs)')
axs[0].legend()
axs[0].grid()

# Wykres P(t)
axs[1].set_title('Moc optyczna P(t)')
line_P, = axs[1].plot(t * 1e6, P_t, label='P(t)', color='orange')
axs[1].set_xlabel('Czas (μs)')
axs[1].legend()
axs[1].grid()

# Wykres I(t)
axs[2].set_title('Prąd fotodetektora I(t)')
line_I_raw, = axs[2].plot(t * 1e6, I_t, label='I(t) - bez filtra', color='red', alpha=0.7)
line_I_filtered, = axs[2].plot(t * 1e6, I_t_filtered, label='I(t) - po filtrze', color='green')
axs[2].set_xlabel('Czas (μs)')
axs[2].legend()
axs[2].grid()
...
freq_slider = Slider(freq_slider_ax, 'Częstotliwość (THz)', 0.01, 0.1, valinit=f_carrier / 1e12)  # Zakres w THz
